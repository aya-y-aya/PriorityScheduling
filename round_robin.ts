...
while (processDone < processes.length) {
    ...
    if (readyQueueIndex >= 0) {
        ...
        if (ongoingProcess > timeQuanta) {
            ...
            if (
                processes[readyQueue[readyQueueIndex]].getFirstArrivalTime() < 0
            ) {
                processes[readyQueue[readyQueueIndex]].setFirstArrivalTime(
                    processTime
                );
            }

            processes[readyQueue[readyQueueIndex]].pushToArrivalTimes(
                processTime
            );

            const remainingProcess =
                processes[readyQueue[readyQueueIndex]].updateRemainingTime(
                    timeQuanta
                );
            ...
            if (!processes[readyQueue[readyQueueIndex]].getIsDoneProcessing()) {
                ...
                processes[readyQueue[readyQueueIndex]].computeValues(
                    processTime
                );
            }

            processes[readyQueue[readyQueueIndex]].pushToCompletionTimes(
                processTime
            );
